...
install = False
if install:
    package = ["fastapi", "uvicorn", "fpdf", "json2pdf_converter", "flask", "python-multipart", "torch", "torchaudio", "deepfilternet"]
    for x in package:
...
def get_data_set(patient_number):
    ret = manager.query_fetch("SELECT date_time, data FROM session WHERE patient_number='" + patient_number + "' ORDER BY date_time DESC;")
    return ret
...
async def save_recording(file: UploadFile = File(...)):
    ...
    with open("input.wav", "wb") as f:
        f.write(contents)
    os.environ["OPENAI_API_KEY"] = "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"
