...
def add_template(filename):
    ...
    query("INSERT INTO template(name, data) values('" + filename.stem + "', '" + temp + "');")
...
def save_template_json(name, json_object):
    query("INSERT INTO template(name, data) values('" + name + "', '" + json.dumps(json_object) +  "');")

def save_session(patient_number, data):
    query("INSERT INTO session(patient_number, data) values('" + patient_number + "', '" + json.dumps(data) + "');")
    return
...
def get_session_meta(patient_number):
    return query_fetch("SELECT id, date_time FROM session WHERE patient_number='" + patient_number + "';")
