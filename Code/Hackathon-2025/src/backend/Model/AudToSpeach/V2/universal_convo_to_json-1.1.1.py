...
How to run (Windows example):
  python universal_convo_to_json.py ^
...
TO RUN SCRIPT:
python src\backend\Model\AudToSpeach\V2\universal_convo_to_json-1.1.1.py --audio "resources\audio\consultation_x1_combined_dialogue.mp3" --template "src\backend\Model\Templates\clinical_note_template.json" --output "src\backend\Model\Output\clinical_note_filled.json" --whisper-model base --diarize --role-map SPEAKER_00=Doctor SPEAKER_01=Patient


If you don't have diarization set up yet, just drop --diarize.
"""
    ...
    for i, p in enumerate(parts):
        ...
        if idx_match:
            ...
            if not isinstance(cur, list):
                raise ValueError(f"Path expects list at segment {p} for {path}")
            while len(cur) <= idx:
                cur.append({})
            if last:
                cur[idx] = value
            ...
            if last:
                if isinstance(cur, list):
                    raise ValueError(f"Cannot set key {p} on list for path {path}")
                cur[p] = value
            else:
                if isinstance(cur, list):
                    raise ValueError(f"Cannot descend key {p} on list for path {path}")
...
def _set_path(root: Dict[str, Any], path: str, value: Any) -> None:
    """
    A safer wrapper that creates dict containers as needed.
    (This version expects only dicts in your provided template paths.)
    """
    parts = path.split(".")
    cur = root
    for i, p in enumerate(parts):
