...
# MODE 2: Train a style profile; supports text-only bootstrap
  # If your train dir has only .txt files, add --bootstrap (and optionally --bootstrap-save)
  python src/backend/Model/AudToSpeach/V2/universal_convo_to_json-1.2.0.py `
    --mode 2 `
...
python src\backend\Model\AudToSpeach\V2\universal_convo_to_json-1.2.0.py --mode 2 --train-dir "src\backend\Model\Training\examples" --model-name "myclinic_v1" --bootstrap --bootstrap-save --audio "resources\audio\consultation_x1_combined_dialogue.mp3" --template "src\backend\Model\Templates\clinical_note_template.json" --output "src\backend\Model\Output\clinical_note_filled.json" --whisper-model base --diarize --role-map SPEAKER_00=Doctor SPEAKER_01=Patient --use-brain --brain-provider openai --brain-model gpt-4o-mini --use-llm --llm-provider openai --llm-model gpt-4o-mini
...
python src\backend\Model\AudToSpeach\V2\universal_convo_to_json-1.2.0.py --mode 3 --load-model "myclinic_v1" --audio "resources\audio\consultation_x1_combined_dialogue.mp3" --template "src\backend\Model\Templates\clinical_note_template.json" --output "src\backend\Model\Output\clinical_note_filled.json" --whisper-model base --diarize --role-map SPEAKER_00=Doctor SPEAKER_01=Patient --use-brain --brain-provider openai --brain-model gpt-4o-mini --use-llm --llm-provider openai --llm-model gpt-4o-mini
...
def main():
    ap = argparse.ArgumentParser(
        description="Audio + Template -> Filled JSON (Brain + optional LLM + trainable STYLE profiles + text-only bootstrap)"
    )
    # Numeric mode
    ap.add_argument(
        "--mode",
