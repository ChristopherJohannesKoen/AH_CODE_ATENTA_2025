...
def json_sections(filled: Dict[str, Any]) -> Dict[str, str]:
    ...
    add(
        ...
        cn.get("past_medical_history") or (filled or {}).get("past_medical_history"),
    )
    ...
    if isinstance(cn.get("vitals"), dict):
        out["vitals"] = " ".join(
            f"{k}: {v}" for k, v in cn["vitals"].items() if v is not None
        )
...
def evaluate_pair(
    ...
    gt_trans_path: str,
    model_json_path: str,
    model_transcript_path: str,
) -> Dict[str, Any]:
    # Load files
    with open(gt_note_path, "r", encoding="utf-8") as f:
        gt_note = f.read()
...
def run_eval(
    ...
    common = sorted(
        set(gt_notes) & set(gt_tx) & set(mdl_json) & set(mdl_tx), key=lambda x: int(x)
    )
    missing = {
    ...
    if common:
        ...
        print(
            " No complete pairs found. Check your directories and file stems (x1..x20)."
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="Objective evaluation of transcripts & clinical notes across paired sessions."
    )
    ap.add_argument(
        "--gt-notes-dir", required=True, help="Directory with reduced_note_x#.txt"
    )
    ap.add_argument(
        "--gt-transcripts-dir", required=True, help="Directory with consultation_x#.txt"
    )
    ap.add_argument(
        "--model-json-dir",
        required=True,
        help="Directory with clinical_note_filled_x#.json (or any *.json with x#)",
    )
    ap.add_argument(
        "--model-transcripts-dir",
        required=True,
        help="Directory with clinical_note_filled_transcript_x#.txt (or any *.txt with x#)",
    )
    ap.add_argument(
        "--out-dir", default="eval_outputs", help="Directory to write results"
    )
    args = ap.parse_args()
