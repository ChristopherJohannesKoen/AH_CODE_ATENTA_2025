...
class StyleTrainer:
    ...
    def _distill_prompt(
        self, examples: List[Tuple[str, Dict[str, Any]]], template: Dict[str, Any]
    ) -> Tuple[str, str]:
        system = (
...
def collect_training_examples(
    ...
    def load_pair(
        txt_path: str, json_path: Optional[str]
    ...
    if subdirs:
        for d in sorted(subdirs):
            ...
            if txts:
                ...
                json_path = (
                    same if os.path.isfile(same) else (jsons[0] if jsons else None)
                )
...
def save_style_profile(
    model_store: str, model_name: str, style_text: str, template: Dict[str, Any]
) -> str:
...
def render_clinical_note(filled: dict) -> str:
    ...
    lines.append(
        f"Medical Record #: {_as_text(patient.get('medical_record_number'))}\n"
    )
    lines.append(f"Date of clinic visit: {_as_text(enc.get('date_of_visit'))}\n")
    lines.append(
        f"Primary care provider: {_as_text(enc.get('primary_care_provider'))}\n"
    )
    lines.append(f"Personal note: {_as_text(enc.get('personal_note'))}\n")

    lines.append(
        f"History of Present Illness: {_as_text(note.get('history_of_present_illness'))}\n"
    )
    ...
    lines.append(
        f"Past Surgical History: {_as_text(prev.get('past_surgical_history'))}"
    )
    ...
    if vitals.get("temperature") is not None:
        vs.append(f"Temp: {_as_text(vitals.get('temperature'))}")
    if vitals.get("blood_pressure") is not None:
        vs.append(f"BP: {_as_text(vitals.get('blood_pressure'))}")
    if vitals.get("heart_rate") is not None:
        vs.append(f"HR: {_as_text(vitals.get('heart_rate'))}")
    if vitals.get("respiratory_rate") is not None:
        vs.append(f"RR: {_as_text(vitals.get('respiratory_rate'))}")
    if vitals.get("oxygen_saturation") is not None:
        vs.append(f"SpO2: {_as_text(vitals.get('oxygen_saturation'))}")
    ...
    lines.append(
        f"Medical Decision Making: {_as_text(note.get('medical_decision_making'))}"
    )
...
def process_audio_to_template(
    ...
    if mode == 2:
        ...
        if not style_guide_text:
            raise FileNotFoundError(
                f"Saved style profile '{load_model}' not found in {model_store}."
            )
    ...
    if use_llm:
        ...
        filled = llm.extract(
            convo_lines, template, brain_text=brain_text, style_guide=style_guide_text
        )
        evidence = {}
    ...
    if style_guide_text and used_style_profile_name:
        print(
            f"[OK] Style profile applied -> {used_style_profile_name} ({model_store})"
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="Audio + Template -> Filled JSON or Plain-Text Clinical Note (Brain + optional LLM + trainable STYLE profiles + text-only bootstrap)"
    )
    # Numeric mode
    ap.add_argument(
        "--mode",
    ...
    ap.add_argument(
        "--output",
        required=True,
        help="Path to output (text note by default; see --output-format)",
    )
    ap.add_argument(
        "--whisper-model",
        default="base",
    ...
    ap.add_argument(
        "--train-dir",
        default=None,
        help="(Mode 2) Directory containing training examples (.txt + .json); with --bootstrap, .txt only is allowed.",
    )
    ap.add_argument(
        "--model-name",
        default=None,
        help="(Mode 2) Name to save the trained style profile under.",
    )
    ap.add_argument(
        "--load-model",
        default=None,
        help="(Mode 3) Name of previously saved style profile to load.",
    )
    ap.add_argument(
        "--model-store",
        default="src/backend/Model/Trained/style_profiles",
        help="Directory to store/load style profiles.",
    )
    ap.add_argument(
        "--max-train-examples",
        type=int,
    ...
    ap.add_argument(
        "--bootstrap",
        action="store_true",
        help="(Mode 2) Allow training from .txt-only examples by auto-generating JSON labels.",
    )
    ap.add_argument(
        "--bootstrap-save",
        action="store_true",
        help="(Mode 2) Save generated labels as *.boot.json alongside the .txt files.",
    )
    ap.add_argument(
        "--bootstrap-provider",
        default="openai",
        help="(Mode 2) Provider to use for bootstrap labeling (default: openai).",
    )
    ap.add_argument(
        "--bootstrap-model",
        default="gpt-4o-mini",
        help="(Mode 2) Model to use for bootstrap labeling (default: gpt-4o-mini).",
    )
